...
- **진입은 SSH guest 전용** (`ssh guest@localhost -p 2222`). `docker exec`(=root) 진입은 실습·채점 무효(0점).
- **플래그 문자열만 제출하면 0점** — Level 3는 익스플로잇 코드+분석 보고서로 채점(플래그는 본인 확인용 토큰).
- 타 학생의 플래그·오프라인 계산 값 제출은 **부정행위로 자동 탐지**됩니다.
